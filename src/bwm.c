...
#define TEXTW(X)                (brw_fontset_getwidth(brw, (X)) + lrpad)
...
/* Vanitygaps forward declarations */
...
cleanup(void)
{
	...
	while (mons)
		cleanupmon(mons);
	for (i = 0; i < CurLast; i++)
		brw_cur_free(brw, cursor[i]);
	for (i = 0; i < LENGTH(colors); i++)
		brw_scm_free(brw, scheme[i], 3);
	free(scheme);
...
{
	int x, w, tw = 0;
	int boxs = brw->fonts->h / 9;
	int boxw = brw->fonts->h / 6 + 2;
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		brw_setscheme(brw, scheme[SchemeNorm]);
		tw = TEXTW(stext) - lrpad + 2; /* 2px right padding */
		brw_text(brw, m->ww - tw, 0, tw, bh, 0, stext, 0);
	}
	...
	w = TEXTW(m->ltsymbol);
	brw_setscheme(brw, scheme[SchemeNorm]);
	x = brw_text(brw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);

	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			brw_setscheme(brw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
			brw_text(brw, x, 0, w, bh, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
				brw_rect(brw, x + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
			brw_setscheme(brw, scheme[SchemeNorm]);
			brw_rect(brw, x, 0, w, bh, 1, 1);
		}
	}
	brw_map(brw, m->barwin, 0, 0, m->ww, bh);
}
...
setup(void)
{
	...
	brw = brw_create(dpy, screen, root, sw, sh);
	if (!brw_fontset_create(brw, fonts, LENGTH(fonts)))
	...
	/* init cursors */
	cursor[CurNormal] = brw_cur_create(brw, XC_left_ptr);
	cursor[CurResize] = brw_cur_create(brw, XC_sizing);
	cursor[CurMove] = brw_cur_create(brw, XC_fleur);
	/* init appearance */
	scheme = ecalloc(LENGTH(colors), sizeof(Clr *));
	for (i = 0; i < LENGTH(colors); i++)
		scheme[i] = brw_scm_create(brw, colors[i], 3);
	/* init bars */
	updatebars();
	updatestatus();
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "bwm", 3);
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
		...
		die("bwm: execvp '%s' failed:", ((char **)arg->v)[0]);
	}
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "bwm: fatal error: request code=%d, error code=%d\n",
...
{
	die("bwm: another window manager is already running");
	return -1;
}
